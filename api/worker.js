...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      if (!Array.isArray(usernames)) throw new Error('Not an array');
    } catch (parseErr) {
      // Fallback: comma-separated string (legacy format)
      if (typeof job === 'string' && job.includes(',')) {
        usernames = job.split(',').map(u => u.trim());
        console.log(`[worker] Parsed legacy format: ${usernames.length} usernames`);
      } else {
