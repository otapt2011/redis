...
export default async function handler(req, res) {
  ...
  try {
    ...
    let usernames;
    // Handle both string (JSON) and direct array (legacy)
    ...
    // ========== CLEANUP: Set TTL when job is fully complete ==========
    ...
    if (queueLength === 0) {
      ...
      if (finalStatusRaw) {
        ...
        if (finalStatus.pending === 0 && finalStatus.completed === finalStatus.total) {
          // All batches processed – set expiry (1 hour) and delete queue key
          await redis.expire(STATUS_KEY, 3600);   // 1 hour
          await redis.expire(RESULTS_KEY, 3600);  // 1 hour
          await redis.del(QUEUE_KEY);             // queue should be empty, but delete it anyway
