...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const item of resultsRaw) {
      // Skip the literal "[object Object]" string (corrupted entry)
      if (typeof item === 'string' && (item === '[object Object]' || item.trim() === '[object Object]')) {
        console.warn('[results] Skipping corrupted entry:', item);
        continue;
      }
      ...
      if (typeof item === 'string') {
        try {
          ...
          console.warn(`[results] Skipping malformed JSON: ${item.substring(0, 100)}`);
        }
      } else {
        console.warn('[results] Unknown item type:', typeof item);
      }
    }
